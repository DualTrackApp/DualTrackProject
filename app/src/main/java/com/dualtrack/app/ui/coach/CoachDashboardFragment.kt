...
class CoachDashboardFragment : Fragment() {
        ...
        ids.forEach { uid ->
            db.collection("users").document(uid)
                ...
                .addOnSuccessListener { doc ->
                    ...
                    remaining--
                    if (remaining == 0) onDone(athletes.sortedBy { it.fullName().lowercase() })
                }
                .addOnFailureListener {
                    remaining--
                    if (remaining == 0) onDone(athletes.sortedBy { it.fullName().lowercase() })
    ...
    private fun calculateAutomaticStatus(athlete: RosterAthlete): StatusInfo {
        if ((athlete.gpa > 0.0 && athlete.gpa < 2.5) ||
            ...
            athlete.attendanceIssues >= 2
        ) {
            val reason = when {
                athlete.missedPractices >= 3 -> "3+ missed practices"
                athlete.missedAssignments >= 3 -> "3+ missed assignments"
                athlete.attendanceIssues >= 2 -> "Attendance issues"
                athlete.gpa > 0.0 && athlete.gpa < 2.5 -> "GPA below 2.5"
                else -> "High risk"
            }
            return StatusInfo("Red", reason)
        }

        if ((athlete.gpa > 0.0 && athlete.gpa < 3.0) ||
            athlete.missedPractices >= 2 ||
            athlete.missedAssignments >= 2 ||
            athlete.attendanceIssues >= 1
        ) {
            val reason = when {
                athlete.missedPractices >= 2 -> "2 missed practices"
                athlete.missedAssignments >= 2 -> "2 missed assignments"
                athlete.attendanceIssues >= 1 -> "Attendance concern"
                athlete.gpa > 0.0 && athlete.gpa < 3.0 -> "GPA below 3.0"
                else -> "At risk"
            }
            return StatusInfo("Yellow", reason)
        }
    ...
    private fun fetchRosterAthletes(onDone: (List<RosterAthlete>) -> Unit) {
        ...
        db.collection("teams").document(teamId)
            ...
            .addOnSuccessListener { rosterSnap ->
                ...
                fetchRosterAthletesByIds(ids, onDone)
            }
            .addOnFailureListener {
                onDone(emptyList())
            }
    ...
    private fun showSpecificPlayersDialog(formType: String, athletes: List<RosterAthlete>) {
        val labels = athletes.map { "${it.fullName()} (${it.email})" }.toTypedArray()
    ...
    private fun showAddAthleteDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton("Assign") { _, _ ->
                val athleteEmail = input.text.toString().trim()
                if (athleteEmail.isBlank()) return@setPositiveButton
                assignAthleteByEmail(athleteEmail, teamId, teamName, coachId)
            }
            .show()
    }
    ...
    private fun assignAthleteByEmail(
        ...
        db.collection("users")
            .whereEqualTo("email", athleteEmail)
            .limit(1)
            .get()
            .addOnSuccessListener { query ->
                if (query.isEmpty) {
                    Toast.makeText(requireContext(), "No user found for email: $athleteEmail", Toast.LENGTH_LONG).show()
                    return@addOnSuccessListener
                }

                val userDoc = query.documents.first()
                val athleteUid = userDoc.id
                val role = userDoc.getString("role").orEmpty()

                if (role.isNotBlank() && !role.equals("athlete", ignoreCase = true)) {
                    Toast.makeText(requireContext(), "That account is not an athlete.", Toast.LENGTH_LONG).show()
                    return@addOnSuccessListener
                }
                ...
                db.collection("users").document(athleteUid)
                    ...
                    .addOnSuccessListener {
                        ...
                        db.collection("teams").document(teamId)
                            ...
                            .addOnFailureListener { e ->
                                Toast.makeText(requireContext(), "Roster update failed: ${e.message}", Toast.LENGTH_LONG).show()
                            }
                    }
                    .addOnFailureListener { e ->
                        Toast.makeText(requireContext(), "User update failed: ${e.message}", Toast.LENGTH_LONG).show()
                    }
            }
            .addOnFailureListener { e ->
