...
class AtRiskFormFragment : Fragment() {
    ...
    private fun setupSpinners() {
        ...
        b.spImpactLevel.adapter = ArrayAdapter(
        ...
        b.spFollowUp.adapter = ArrayAdapter(
    ...
    private fun submitAtRisk() {
        ...
        val concernArea = b.etConcernArea.text.toString().trim()

        val incidentDate = b.etIncidentDate.text.toString().trim()

        val observedBehavior = b.etObservedBehavior.text.toString().trim()

        val currentImpact = b.spImpactLevel.selectedItem?.toString()?.trim().orEmpty()

        val actionsTaken = b.etActionsTaken.text.toString().trim()

        val followUpNeeded = b.spFollowUp.selectedItem?.toString()?.trim().orEmpty()
        ...
        if (concernArea.isBlank() || incidentDate.isBlank() || observedBehavior.isBlank() || message.isBlank()) {

            Toast.makeText(requireContext(), "Please complete the required at-risk fields.", Toast.LENGTH_SHORT).show()
        ...
        if (currentImpact == "Select impact level" || followUpNeeded == "Select follow-up needed") {

            Toast.makeText(requireContext(), "Please select impact level and follow-up needed.", Toast.LENGTH_SHORT).show()
            ...
            .addOnSuccessListener { snap ->
                ...
                val formData = hashMapOf(
                    ...
                    "data" to mapOf(
                        ...
                        "concernArea" to concernArea,

                        "incidentDate" to incidentDate,

                        "observedBehavior" to observedBehavior,

                        "impactLevel" to currentImpact,

                        "actionsTaken" to actionsTaken,

                        "followUpNeeded" to followUpNeeded,

                        "summaryMessage" to message
